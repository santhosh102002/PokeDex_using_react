    ...
    return(
     ...
     {pokemon && <div className="pokemon-details-wrapper">
            ...
            <div className="pokemon-types">
              <h1 >Type:</h1>{pokemon.types.map(t=><span className="types" key={t.type.name}>{t.type.name}</span>)}
            </div>
    </div>}
    <div className="similar-pokemons">
        ...
        <div className="pokemon-similar-boxes">
             {pokemonListState.pokemonList.length>0 &&
               pokemonListState.pokemonList.map((pokemon)=><Pokemon name={pokemon.name} key={pokemon.id} url={pokemon.image} id={pokemon.id}></Pokemon>)
             }
        </div>
    </div>
    </>
